...
const RoleDetails = ({ role, isOpen, onClose }) => {
    ...
    const enabledPermissions = Object.entries(permissions).filter(([_, value]) => value === true);
            ...
            <div className="bg-white rounded-lg shadow-xl max-w-4xl w-full max-h-[90vh] overflow-y-auto">
                ...
                <div className="p-6 space-y-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            <label className="text-xs font-semibold text-gray-500 uppercase">Access Level</label>
                            <div className={`mt-1 px-3 py-2 rounded-lg border ${getAccessLevelColor(role.accessLevel)}`}>
                    ...
                    {/* Purpose */}
                    <div>
                        <label className="text-xs font-semibold text-gray-500 uppercase mb-2 block">Purpose</label>
                    ...
                    {role.restrictions && role.restrictions.length > 0 && (
                        <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
                            <div className="flex items-start gap-2 mb-2">
                                ...
                                <h3 className="font-semibold text-yellow-900">What this role can't do:</h3>
                            </div>
                    ...
                    <div>
                        ...
                        <div className="space-y-4 border border-gray-200 rounded-lg p-4">
                            {Object.entries(PERMISSION_GROUPS).map(([groupName, groupPermissions]) => {
                                const groupEnabled = groupPermissions.filter(key => permissions[key] === true);
                    ...
                    {Object.values(permissions).filter(v => v === false).length > 0 && (
                        <div className="bg-gray-50 border border-gray-200 rounded-lg p-4">
                            <p className="text-sm text-gray-600">
                                <strong>{Object.values(permissions).filter(v => v === false).length}</strong> permissions are disabled for this role.
                            </p>
                        </div>
                    )}
